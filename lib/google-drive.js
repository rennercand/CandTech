...
import { consumeOAuthTransaction, createOAuthTransaction } from "./db.js";
...
export async function createDriveState(userId, redirectUri, historyId = null, sessionHash = "", returnTo = "", filename = "", nonce = "") {
  ...
  return new SignJWT({ purpose: "google-drive", redirectUri, historyId, sessionHash, returnTo: safeReturnTo, filename: safeFilename, nonce })
...
export async function verifyDriveState(state) {
  ...
  if (payload.purpose !== "google-drive" || !payload.sub || !payload.redirectUri || !/^[A-Za-z0-9_-]{40,100}$/.test(String(payload.nonce || ""))) {
...
export async function googleAuthorizationUrl({ userId, redirectUri, historyId, sessionHash, returnTo = "", filename = "" }) {
  ...
  await createOAuthTransaction({
    nonceHash: hashOAuthNonce(nonce), userId, sessionHash, provider: "google-drive",
    encryptedCodeVerifier: encryptDriveToken(codeVerifier),
    expiresAt: new Date(Date.now() + 10 * 60 * 1_000),
  });
  const state = await createDriveState(userId, redirectUri, historyId, sessionHash, returnTo, filename, nonce);
...
export async function consumeDriveOAuthTransaction({ nonce, userId, sessionHash }) {
  const encrypted = await consumeOAuthTransaction({
    nonceHash: hashOAuthNonce(nonce), userId, sessionHash, provider: "google-drive",
  });
  return encrypted ? decryptDriveToken(encrypted) : null;
}
