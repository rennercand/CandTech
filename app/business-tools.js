...
export const emptyCommerceOrder = () => ({
  id: "", type: "venda", number: "", partner: "", contact: "", description: "", date: "",
...
function Field({ label, children }) {
  // A pré-nota atual não tem validade fiscal; portanto não solicita documento do cliente.
  if (label === "CPF / CNPJ") return null;
...
export function SalesPurchases({ orders, setOrders, products = [], issuer, setIssuer, onStatusChange, onTestInvoice, onSuggestFromCash }) {
  ...
  <section className="panel commerce-stock-panel"><div className="panel-heading"><div><span className="eyebrow">ESTOQUE CONECTADO</span><h2>Produtos disponíveis para os pedidos</h2><p>Selecione um produto no pedido. Venda concluída reduz o saldo; compra concluída aumenta.</p></div></div>
    ...
    })}</div> : <p className="empty-state">Cadastre produtos em Logística e estoque para vinculá-los aos pedidos.</p>}
  </section>
  <section className="panel operations-panel issuer-panel"><div className="panel-heading"><div><span className="eyebrow">DOCUMENTO COMERCIAL</span><h2>Dados do emitente</h2><p>Usados na pré-nota em PDF. Ela não possui validade fiscal e não exige CPF do cliente.</p></div></div><div className="issuer-grid"><Field label="Razão social / nome"><input value={issuer.legalName} onChange={(e) => setIssuer((current) => ({ ...current, legalName: e.target.value }))} /></Field><Field label="CNPJ da empresa (opcional)"><input inputMode="numeric" value={issuer.document} onChange={(e) => setIssuer((current) => ({ ...current, document: e.target.value }))} /></Field><Field label="Inscrição estadual (opcional)"><input value={issuer.stateRegistration} onChange={(e) => setIssuer((current) => ({ ...current, stateRegistration: e.target.value }))} /></Field><Field label="Endereço"><input value={issuer.address} onChange={(e) => setIssuer((current) => ({ ...current, address: e.target.value }))} /></Field><Field label="Cidade"><input value={issuer.city} onChange={(e) => setIssuer((current) => ({ ...current, city: e.target.value }))} /></Field><Field label="UF"><input maxLength="2" value={issuer.state} onChange={(e) => setIssuer((current) => ({ ...current, state: e.target.value.toUpperCase() }))} /></Field></div></section>
