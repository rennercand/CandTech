...
export async function claimIdempotency({ userId, organizationId = null, operation, keyHash, requestHash, ttlHours = 24, allowReclaim = true }) {
  ...
  if (backend.type === "postgres") {
    ...
    // Efeitos externos incertos (como e-mail) não podem ser repetidos só por timeout.
    if (!allowReclaim) return { state: "pending" };
