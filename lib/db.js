...
export async function getWorkspace(userId, organizationId = null) {
    ...
    const rows = organizationId == null
      ? await backend.sql`
          SELECT user_id, organization_id, payload, revision, archived_revision,
                 clients_relational_at, tasks_relational_at, updated_at
  ...
  return hydrateWorkspaceEntities(backend, serializeWorkspace(
    organizationId == null
      ? backend.db.prepare("SELECT user_id, organization_id, payload, revision, archived_revision, clients_relational_at, tasks_relational_at, updated_at FROM workspaces WHERE user_id = ? AND organization_id IS NULL").get(userId)
      : backend.db.prepare("SELECT user_id, organization_id, payload, revision, archived_revision, clients_relational_at, tasks_relational_at, updated_at FROM workspaces WHERE user_id = ? AND organization_id = ?").get(userId, organizationId),
  ));
}
