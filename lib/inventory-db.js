    ...
    if (backend.type === "sqlite") {
      backend.db.exec(`
        ...
        CREATE TABLE IF NOT EXISTS inventory_orders (
          ...
          reference TEXT NOT NULL DEFAULT '', partner TEXT NOT NULL DEFAULT '', customer_id INTEGER, status TEXT NOT NULL DEFAULT 'completed',
      ...
      for (const [name, definition] of [
        ...
        ["payment_method", "TEXT NOT NULL DEFAULT 'pending'"], ["payment_status", "TEXT NOT NULL DEFAULT 'pending'"], ["due_on", "TEXT"],
...
export async function listInventory(tenantId) {
  ...
  if (backend.type === "postgres") {
    [rows, batches, orders, lots, analyticsRows, deliveries] = await Promise.all([
      ...
      backend.sql`SELECT o.public_id AS id, o.type, o.reference, o.partner, c.public_id AS customer_id, o.status,
        o.subtotal, o.discount_amount, o.total, o.payment_method, o.payment_status, o.due_on, o.created_at
        FROM inventory_orders o LEFT JOIN customers c ON c.id=o.customer_id WHERE o.tenant_id = ${tenantId}
    ...
    orders = backend.db.prepare(`SELECT o.public_id AS id, o.type, o.reference, o.partner, c.public_id AS customer_id, o.status,
      o.subtotal, o.discount_amount, o.total, o.payment_method, o.payment_status, o.due_on, o.created_at
      FROM inventory_orders o LEFT JOIN customers c ON c.id=o.customer_id
...
export async function createInventoryOrder({ tenantId, userId, type, reference, partner, customerId = "", lines, discountAmount = 0,
  ...
  if (backend.type === "postgres") {
    ...
    await backend.sql.transaction((tx) => [
      ...
      tx`SELECT 1 / CASE WHEN COUNT(*) = ${safeLines.length} THEN 1 ELSE COUNT(*) - COUNT(*) END AS valid
        FROM inventory_movements m JOIN inventory_batches b ON b.id = m.batch_id
        WHERE b.public_id = ${batchId} AND b.tenant_id = ${tenantId}`,
      tx`INSERT INTO inventory_orders (public_id, tenant_id, organization_id, type, reference, partner, customer_id, subtotal, discount_amount,
    ...
    try {
      ...
      const order = backend.db.prepare(`INSERT INTO inventory_orders
        (public_id, tenant_id, organization_id, type, reference, partner, customer_id, subtotal, discount_amount, total, payment_method, payment_status, due_on, batch_id, actor_user_id, idempotency_key_hash)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(orderId, tenantId, organizationId, type, reference, partner, customer?.id || null, subtotal, safeDiscount, total, paymentMethod, paymentStatus, dueOn,
