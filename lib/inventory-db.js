  ...
  if (backend.type === "postgres") {
    ...
    await backend.sql.transaction((tx) => [
      ...
      ...(kind === "sale" ? [tx`INSERT INTO operational_deliveries
        (public_id, owner_user_id, organization_id, inventory_order_id, description, partner, direction, status)
        SELECT ${deliveryId}, organization.owner_user_id, inventory_order.organization_id, inventory_order.id,
    ...
    try {
      ...
      if (kind === "sale") {
        ...
        backend.db.prepare(`INSERT INTO operational_deliveries
          (public_id, owner_user_id, organization_id, order_public_id, description, partner, direction, status)
          VALUES (?, ?, ?, ?, ?, ?, 'saida', 'preparando')`)
          .run(deliveryId, owner.owner_user_id, organizationId, orderId, `Entrega ${reference || orderId.slice(0, 8)}`, partner);
      }
