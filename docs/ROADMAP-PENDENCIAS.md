...
- [ ] transformar `audit_events` em trilha suficiente para operações críticas, com autor, organização, origem, versão e antes/depois minimizado; **estrutura v2, minimização e eventos de autenticação, equipe, aceite jurídico, Pix, Drive e exportações implementados; consulta paginada exclusiva da conta raiz com MFA e auditoria do próprio acesso implementada em 02/09/2026; a retenção técnica está em preservação sem exclusão automática até a aprovação dos prazos jurídicos por categoria**;
...
- [ ] implementar idempotência persistida para mutações críticas e uma outbox antes de ampliar integrações e jobs; **histórico, estoque, ordens de serviço e moderação Pix exigem chave persistida, com replay e conflito. Repetir uma aprovação Pix não amplia a assinatura novamente. Venda/compra, baixa/entrada, pedidos e conclusão de serviço usam transações serializáveis e outbox; repetição não movimenta saldo nem cria cobrança novamente. O worker protegido por `CRON_SECRET` possui claim exclusivo, tentativas e backoff. Ainda faltam aplicar o mesmo contrato às demais mutações administrativas críticas**;
...
- [x] tornar o alerta de reposição persistente e configurável por SKU; **a empresa define a quantidade de disparo e uma data opcional, recebe o lembrete em toda nova entrada no sistema e continua vendo um contador vermelho acessível no menu até repor ou reconfigurar o item; alteração e leitura permanecem isoladas por organização**;
...
- [x] relacionar compras e entradas a fornecedores; **o cadastro guarda contato e prazo médio por organização, compras selecionam o fornecedor real, o nome histórico permanece no pedido e a visão mostra quantidade, total e última compra. O teste negativo rejeita fornecedor pertencente a outra empresa; tabela, índice e vínculos foram verificados nas branches Preview e Production do Neon em 02/09/2026**;
