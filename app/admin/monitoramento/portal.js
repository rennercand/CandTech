...
const eventStatus = { open: "Aberto", investigating: "Investigando", resolved: "Resolvido" };
...
export default function MonitoringPortal({ administratorName }) {
  const [data, setData] = useState({ events: [], tickets: [], totals: {}, checkedAt: null });
  ...
  return <main className="monitor-page">
    ...
    <section className="monitor-content">
      ...
      </section> : <section className="monitor-panel"><div className="monitor-panel-heading"><div><h2>Mensagens recebidas</h2><p>Chamados enviados pela aba Suporte do ERP.</p></div><span>{data.tickets.length} registro(s)</span></div>
