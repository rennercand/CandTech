...
- [ ] concluir o isolamento relacional por organização para workspace, históricos, clientes, tarefas, entregas e lançamentos que ainda vivem em payloads agregados; **workspace, histórico, clientes, tarefas, entregas e livro financeiro já possuem escopo relacional, backfill e índices verificados em Preview/Production. No estoque, `organization_id`, backfill e índices foram aplicados nos dois branches; novas escritas preenchem os dois identificadores e o teste cobre duas organizações reais. Falta trocar as leituras para o novo campo e retirar `tenant_id` somente em deploy futuro**;
