export function guardMutation(request, { allowedContentTypes = ["application/json"] } = {}) {
  ...
  // Cada rota declara explicitamente os formatos aceitos. JSON continua sendo
  // o padrão; uploads binários precisam optar pelo MIME esperado.
...
export async function readLimitedBytes(request, { maxBytes }) {
  if (!Number.isSafeInteger(maxBytes) || maxBytes < 1) throw new TypeError("Limite binário inválido.");
  const declaredLength = Number(request.headers.get("content-length") || 0);
  if (declaredLength > maxBytes) throw new RequestBodyError("Conteúdo muito grande.", 413);
  if (!request.body) throw new RequestBodyError("Arquivo ausente.");
