...
export default function InventoryOperations({ initialSection = "overview", onSnapshot, onDeliveriesChange, clients = [], canExport = false, canUseDrive = false, driveStatus }) {
  ...
  return <div className="business-stack relational-inventory">
    ...
    {section === "orders" && <><section className="panel"><div className="panel-heading"><div><span className="eyebrow">COMERCIAL</span><h2>Pedido com vários produtos</h2><p>A venda reduz o estoque pelo lote que vence primeiro; a compra aumenta. Se houver erro, desfaça a movimentação no histórico.</p></div></div><OrderForm variants={variants} busy={busy} onSave={(data) => post({ action: "order", ...data }, `${data.type === "sale" ? "Venda" : "Compra"} concluída e estoque atualizado.`)} /></section>{inventory.orders.length > 0 && <section className="panel"><div className="panel-heading"><div><span className="eyebrow">RECENTES</span><h2>Últimos pedidos</h2></div></div><div className="batch-list">{inventory.orders.map((order) => <div key={order.id}><span><strong>{order.type === "sale" ? "Venda" : "Compra"} · {order.reference || "Sem referência"}</strong><small>{order.partner || "Sem cliente/fornecedor"} · {new Date(order.created_at).toLocaleString("pt-BR")}</small></span><span><strong>{money.format(order.total)}</strong>{order.status === "cancelled" && <small>Cancelado</small>}</span></div>)}</div></section>}</>}
