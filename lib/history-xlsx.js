...
const LABELS = {
  ...
  variation: "Variação", salePrice: "Preço de venda", unit: "Unidade",
  lotCode: "Lote", expiresOn: "Validade", receivedQuantity: "Quantidade recebida",
};
...
const CURRENCY_KEYS = new Set([
  ...
  "principal", "totalInterest", "totalPaid", "salePrice",
]);
const PERCENTAGE_KEYS = new Set(["interestRate", "marginRate"]);
const INTEGER_KEYS = new Set(["period", "periods", "quantity", "minimum", "receivedQuantity"]);
...
function reportSections(item) {
  ...
  const products = (source.inventoryState?.products || [])
    ...
    .map((product) => ({
      ...
      totalCost: (Number(product.quantity) || 0) * (Number(product.unitCost) || 0),
      category: product.category || "", unit: product.unit || "", location: product.location,
    }));
  ...
  const lots = (source.inventoryLots || []).filter((lot) => lot.lotCode || lot.expiresOn);
  if (lots.length) sections.push({ title: "Lotes e validades recebidos", rows: lots });
  return sections;
}
