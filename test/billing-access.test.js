...
import {
  ...
  ensureOwnedOrganization, updateStripeSubscription,
} from "../lib/db.js";
...
test("pagamento do proprietário libera a equipe e a trava pode ser ativada com segurança", async () => {
  ...
  try {
    ...
    await updateStripeSubscription({ userId: owner.id, customerId: "cus_owner", subscriptionId: "sub_owner", priceId: "price_test", status: "active" });
