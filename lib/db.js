    ...
    const rows = organizationId == null
      ? await backend.sql`
          SELECT user_id, organization_id, payload, revision, archived_revision,
                 clients_relational_at, tasks_relational_at, deliveries_relational_at, updated_at
      ...
      : await backend.sql`
        ...
        `;
    return hydrateWorkspaceEntities(backend, serializeWorkspace(rows[0]));
  }
  return hydrateWorkspaceEntities(backend, serializeWorkspace(
    organizationId == null
      ? backend.db.prepare("SELECT user_id, organization_id, payload, revision, archived_revision, clients_relational_at, tasks_relational_at, deliveries_relational_at, updated_at FROM workspaces WHERE user_id = ? AND organization_id IS NULL").get(userId)
      : backend.db.prepare("SELECT user_id, organization_id, payload, revision, archived_revision, clients_relational_at, tasks_relational_at, deliveries_relational_at, updated_at FROM workspaces WHERE user_id = ? AND organization_id = ?").get(userId, organizationId),
  ));
}
