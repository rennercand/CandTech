...
export async function createDriveState(userId, redirectUri) {
  // O state assinado vincula o retorno do Google ao usuário e impede CSRF no callback.
  return new SignJWT({ purpose: "google-drive", redirectUri })
...
export async function googleAuthorizationUrl({ userId, redirectUri }) {
  const state = await createDriveState(userId, redirectUri);
