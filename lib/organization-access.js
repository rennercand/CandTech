...
export function isPublicHistoryId(value) {
  return typeof value === "string" && /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value);
}

/**
 * Resolve no servidor a organização e o proprietário real dos dados. Rotas não
 * devem aceitar ownerUserId ou tenantId do navegador como fonte de autoridade.
 */
...
/** Retorna o escopo empresarial somente quando a permissão atual está ativa. */
...
/**
 * Combina o UUID público com o proprietário derivado da sessão. Essa dupla
 * condição é a defesa efetiva contra troca de ID/IDOR entre empresas.
 */
export async function getAccessibleHistory({ user, id, permissions = ["history"] }) {
  ...
  if (!isPublicHistoryId(id)) return { access, item: null, forbidden: false };
  const row = await findHistoryById(id, access.ownerUserId, access.organizationId);
