...
import { INVENTORY_TEMPLATE, parseInventoryFile, parseInventoryText } from "@/lib/inventory-import";
...
function ImportPanel({ onImport, busy }) {
  ...
  return <div className="inventory-form-stack"><div className="import-options"><label className="file-drop"><strong>Selecionar planilha</strong><span>CSV, TSV, TXT ou XLSX</span><input type="file" accept=".csv,.tsv,.txt,.xlsx" onChange={(e) => fileChanged(e.target.files?.[0])} /></label><div><strong>Ou cole linhas do Excel</strong><textarea rows="7" value={pasted} onChange={(e) => setPasted(e.target.value)} placeholder={INVENTORY_TEMPLATE} /><button type="button" className="secondary-button" onClick={parsePaste}>Preparar prévia</button></div></div>
    ...
    {preview && <div className="import-preview"><strong>Prévia antes de gravar</strong><span>{preview.variantCount} SKUs em {preview.products.length} produtos</span><small>Nenhuma linha será gravada até sua confirmação.</small><button className="primary-button" disabled={busy || preview.errors.length > 0} onClick={() => onImport(preview.products)}>{busy ? "Importando…" : "Confirmar importação"}</button></div>}
  </div>;
}

export default function InventoryOperations({ initialSection = "overview", onSnapshot }) {
  ...
  return <div className="business-stack relational-inventory">
    ...
    {section === "overview" && <><div className="summary-grid"><article><span>Produtos</span><strong>{inventory.products.length}</strong><small>{variants.length} SKUs</small></article><article><span>Unidades em estoque</span><strong>{totalUnits}</strong><small>Somadas em todas as variações</small></article><article><span>Valor pelo custo</span><strong>{money.format(totalValue)}</strong><small>Estimativa operacional</small></article><article><span>Alertas</span><strong>{low.length}</strong><small>No mínimo ou sem estoque</small></article></div><section className="panel"><div className="panel-heading"><div><span className="eyebrow">PRÓXIMA AÇÃO</span><h2>Atalhos para o funcionário</h2></div></div><div className="inventory-shortcuts"><button onClick={() => setSection("entry")}><strong>Mercadoria chegou</strong><span>Registrar entrada de vários itens</span></button><button onClick={() => setSection("orders")}><strong>Venda ou compra</strong><span>Montar pedido com vários produtos</span></button><button onClick={() => setSection("import")}><strong>Estoque inicial</strong><span>Importar planilha com prévia</span></button></div></section>{low.length > 0 && <section className="panel"><div className="panel-heading"><div><span className="eyebrow">ATENÇÃO</span><h2>Estoque baixo</h2></div></div><div className="stock-alert-list">{low.map((item) => <div key={item.id}><strong>{item.product.name} · {item.name}</strong><span>{item.quantity} {item.product.unit} · mínimo {item.minimumQuantity}</span></div>)}</div></section>}</>}
    ...
    {section === "import" && <section className="panel"><div className="panel-heading"><div><span className="eyebrow">CADASTRO EM LOTE</span><h2>Importar estoque inicial</h2><p>Revise a prévia. SKUs repetidos são recusados para impedir que o produto errado seja movimentado.</p></div></div><ImportPanel busy={busy} onImport={(products) => post({ action: "import-products", products, reference: "Importação por planilha" }, "Planilha importada e saldo inicial registrado.")} /></section>}
    ...
    {section === "guide" && <section className="panel employee-guide"><div className="panel-heading"><div><span className="eyebrow">CAPACITAÇÃO</span><h2>Rotina simples para o funcionário</h2><p>Use esta sequência no treinamento. O funcionário não precisa conhecer banco de dados nem cálculos.</p></div></div><ol><li><strong>Produto novo:</strong> abra “Produtos e variações”, cadastre o nome e crie um SKU para cada modelo, cor, sabor ou tamanho.</li><li><strong>Mercadoria recebida:</strong> abra “Entrada rápida”, escolha os itens, informe quantidade e custo e confirme uma vez.</li><li><strong>Venda ou compra:</strong> abra “Pedidos”, adicione todos os produtos e confira o total antes de concluir.</li><li><strong>Estoque inicial grande:</strong> baixe o modelo em “Importar planilha”, preencha e confira a prévia.</li><li><strong>Erro operacional:</strong> não altere o saldo manualmente; abra “Movimentações” e use “Desfazer”.</li><li><strong>Alimentos:</strong> sempre informe lote e validade na entrada para permitir rastreamento.</li></ol><div className="training-rule"><strong>Regra principal</strong><span>SKU identifica o item. Quantidade muda por entrada, compra, venda ou desfazimento — nunca por edição escondida.</span></div></section>}
  </div>;
}
