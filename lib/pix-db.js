...
export async function createOrGetPixPaymentRequest(userId) {
  ...
  const setupPaid = backend.type === "postgres"
    ? Boolean((await backend.sql`SELECT
        EXISTS (SELECT 1 FROM billing_profiles WHERE user_id=${userId} AND setup_paid_at IS NOT NULL)
        OR EXISTS (SELECT 1 FROM pix_payment_requests WHERE user_id=${userId} AND kind='initial' AND status='approved')
        AS setup_paid`)[0].setup_paid)
    : Boolean(backend.db.prepare(`SELECT
        EXISTS (SELECT 1 FROM billing_profiles WHERE user_id=? AND setup_paid_at IS NOT NULL)
        OR EXISTS (SELECT 1 FROM pix_payment_requests WHERE user_id=? AND kind='initial' AND status='approved')
        AS setup_paid`).get(userId, userId).setup_paid);
  const amounts = pixAmounts();
  const kind = setupPaid ? "renewal" : "initial";
...
export async function getActivePixReceiptForAdmin(id) {
  ...
  if (!row) return null;
  return {
    ...serializeReceipt(row, { includeStorage: true }),
    paymentId: row.payment_public_id,
    userId: Number(row.payment_user_id),
