...
import {
  hydrateWorkspaceEntities,
  listWorkspaceCustomers as listRelationalCustomers,
  listWorkspaceDeliveries as listRelationalDeliveries,
  listWorkspaceTasks as listRelationalTasks,
  syncWorkspaceEntities,
} from "./db/workspace-entities.js";
import {
  hydrateWorkspaceFinance,
  initializeSqliteFinance,
  listFinancialCommitments as listRelationalFinancialCommitments,
  listFinancialLedgerEntries as listRelationalFinancialEntries,
  syncWorkspaceFinance,
} from "./db/finance.js";
...
// O SQLite local ainda repara bancos antigos na inicialização. No Postgres,
// a mesma estratégia vive exclusivamente na migration versionada.
...
async function createPostgresBackend() {
  ...
  // O runtime nunca altera a estrutura do Neon. DDL e reparos de dados são
  // executados uma única vez pelos arquivos versionados em migrations/.
...
async function createSqliteBackend() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS workspaces (
      ...
      clients_relational_at TEXT,
      tasks_relational_at TEXT,
      deliveries_relational_at TEXT,
    ...
    CREATE TABLE IF NOT EXISTS operational_deliveries (
      ...
      planned_for TEXT,
      status TEXT NOT NULL DEFAULT 'preparando' CHECK(status IN ('preparando', 'em-transito', 'entregue', 'cancelada')),
    ...
    CREATE INDEX IF NOT EXISTS idx_outbox_events_pending ON outbox_events(status, available_at, id);
    ...
    -- A equipe interna usa o mesmo login verificado dos usuários, mas recebe
    -- somente permissões administrativas explícitas. Senhas nunca são criadas
    -- ou armazenadas por esta tabela.
    CREATE TABLE IF NOT EXISTS staff_access (
      user_id INTEGER PRIMARY KEY,
      can_monitor INTEGER NOT NULL DEFAULT 0 CHECK(can_monitor IN (0,1)),
      can_support INTEGER NOT NULL DEFAULT 0 CHECK(can_support IN (0,1)),
      can_billing INTEGER NOT NULL DEFAULT 0 CHECK(can_billing IN (0,1)),
  ...
  const sessionColumns = db.prepare("PRAGMA table_info(auth_sessions)").all();
  if (!sessionColumns.some((column) => column.name === "mfa_verified_at")) {
    db.exec("ALTER TABLE auth_sessions ADD COLUMN mfa_verified_at TEXT");
  }
  ...
  if (!billingColumns.some((column) => column.name === "setup_paid_at")) {
    db.exec("ALTER TABLE billing_profiles ADD COLUMN setup_paid_at TEXT");
  }
  ...
  if (!historyColumns.some((column) => column.name === "organization_id")) {
    db.exec("ALTER TABLE histories ADD COLUMN organization_id INTEGER");
  }
  ...
  if (!workspaceColumns.some((column) => column.name === "organization_id")) {
    db.exec("ALTER TABLE workspaces ADD COLUMN organization_id INTEGER");
  }
  if (!workspaceColumns.some((column) => column.name === "clients_relational_at")) {
    db.exec("ALTER TABLE workspaces ADD COLUMN clients_relational_at TEXT");
  }
  if (!workspaceColumns.some((column) => column.name === "tasks_relational_at")) {
    db.exec("ALTER TABLE workspaces ADD COLUMN tasks_relational_at TEXT");
  }
  if (!workspaceColumns.some((column) => column.name === "deliveries_relational_at")) {
    db.exec("ALTER TABLE workspaces ADD COLUMN deliveries_relational_at TEXT");
  }
  initializeSqliteFinance(db);
  db.exec(`
    UPDATE histories
    SET organization_id = (SELECT id FROM organizations WHERE owner_user_id = histories.user_id)
    WHERE organization_id IS NULL AND EXISTS (SELECT 1 FROM organizations WHERE owner_user_id = histories.user_id);
    UPDATE workspaces
    SET organization_id = (SELECT id FROM organizations WHERE owner_user_id = workspaces.user_id)
    WHERE organization_id IS NULL AND EXISTS (SELECT 1 FROM organizations WHERE owner_user_id = workspaces.user_id);
    CREATE INDEX IF NOT EXISTS idx_histories_organization_created ON histories(organization_id, id DESC);
    CREATE INDEX IF NOT EXISTS idx_workspaces_organization ON workspaces(organization_id);
  `);

  const auditColumns = db.prepare("PRAGMA table_info(audit_events)").all();
  const auditColumnDefinitions = {
    actor_user_id: "INTEGER",
    organization_id: "INTEGER",
    origin: "TEXT NOT NULL DEFAULT 'application'",
    event_version: "INTEGER NOT NULL DEFAULT 2",
  ...
  for (const [column, definition] of Object.entries(auditColumnDefinitions)) {
    if (!auditColumns.some((existing) => existing.name === column)) {
      db.exec(`ALTER TABLE audit_events ADD COLUMN ${column} ${definition}`);
    }
  }
  db.exec(`
...
export async function createAuthSession({ sessionHash, userId, expiresAt, mfaVerified = false }) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    await backend.sql`
      INSERT INTO auth_sessions (session_hash, user_id, expires_at, mfa_verified_at)
      VALUES (${sessionHash}, ${userId}, ${expiresAt.toISOString()}, ${mfaVerified ? new Date().toISOString() : null})
    `;
    if (Math.random() < 0.02) {
  ...
  backend.db.prepare("INSERT INTO auth_sessions (session_hash, user_id, expires_at, mfa_verified_at) VALUES (?, ?, ?, ?)")
    .run(sessionHash, userId, expiresAt.toISOString(), mfaVerified ? new Date().toISOString() : null);
  if (Math.random() < 0.02) {
...
export async function findActiveAuthSession(sessionHash) {
  ...
  if (backend.type === "postgres") {
    const rows = await backend.sql`
      SELECT user_id, expires_at, last_seen_at, mfa_verified_at FROM auth_sessions
    ...
    return rows[0] ? { userId: Number(rows[0].user_id), expiresAt: rows[0].expires_at, mfaVerifiedAt: rows[0].mfa_verified_at } : null;
  }
  const row = backend.db.prepare(
    "SELECT user_id, expires_at, last_seen_at, mfa_verified_at FROM auth_sessions WHERE session_hash = ? AND revoked_at IS NULL AND datetime(expires_at) > CURRENT_TIMESTAMP",
  ).get(sessionHash);
  if (!row) return null;
  ...
  return { userId: Number(row.user_id), expiresAt: row.expires_at, mfaVerifiedAt: row.mfa_verified_at };
}

export async function markAuthSessionMfaVerified({ sessionHash, userId, revokeOthers = false }) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    await backend.sql`UPDATE auth_sessions SET mfa_verified_at = NOW()
      WHERE session_hash = ${sessionHash} AND user_id = ${userId} AND revoked_at IS NULL`;
    if (revokeOthers) await backend.sql`UPDATE auth_sessions SET revoked_at = NOW()
      WHERE user_id = ${userId} AND session_hash <> ${sessionHash} AND revoked_at IS NULL`;
    return;
  }
  backend.db.exec("BEGIN IMMEDIATE");
  try {
    backend.db.prepare("UPDATE auth_sessions SET mfa_verified_at = CURRENT_TIMESTAMP WHERE session_hash = ? AND user_id = ? AND revoked_at IS NULL")
      .run(sessionHash, userId);
    if (revokeOthers) backend.db.prepare("UPDATE auth_sessions SET revoked_at = CURRENT_TIMESTAMP WHERE user_id = ? AND session_hash <> ? AND revoked_at IS NULL")
      .run(userId, sessionHash);
...
const AUDIT_SECRET_FIELD = /(password|passphrase|secret|token|authorization|cookie|credential|private.?key|pix.?key|refresh.?token|access.?token)/i;
...
function sanitizeAuditValue(value, depth = 0) {
  ...
  if (typeof value === "object") {
    return Object.fromEntries(Object.entries(value).slice(0, 40).map(([key, item]) => [
      ...
      AUDIT_SECRET_FIELD.test(key) ? "[redacted]" : sanitizeAuditValue(item, depth + 1),
    ]));
  }
  return String(value).slice(0, 500);
}

export async function createOAuthTransaction({ nonceHash, userId, sessionHash, provider, encryptedCodeVerifier, expiresAt }) {
  const backend = await getBackend();
  const safeProvider = String(provider).slice(0, 40);
  if (backend.type === "postgres") {
    await backend.sql`
      UPDATE oauth_transactions SET used_at = NOW()
      WHERE user_id = ${userId} AND session_hash = ${sessionHash} AND provider = ${safeProvider} AND used_at IS NULL
    `;
    await backend.sql`
      INSERT INTO oauth_transactions (nonce_hash, user_id, session_hash, provider, encrypted_code_verifier, expires_at)
      VALUES (${nonceHash}, ${userId}, ${sessionHash}, ${safeProvider}, ${encryptedCodeVerifier}, ${expiresAt.toISOString()})
    `;
    if (Math.random() < 0.05) {
      await backend.sql`DELETE FROM oauth_transactions WHERE expires_at < NOW() - INTERVAL '1 day'`;
    }
    return;
  }
  backend.db.exec("BEGIN IMMEDIATE");
  try {
    backend.db.prepare(`UPDATE oauth_transactions SET used_at = CURRENT_TIMESTAMP
      WHERE user_id = ? AND session_hash = ? AND provider = ? AND used_at IS NULL`)
      .run(userId, sessionHash, safeProvider);
    backend.db.prepare(`INSERT INTO oauth_transactions
      (nonce_hash, user_id, session_hash, provider, encrypted_code_verifier, expires_at)
      VALUES (?, ?, ?, ?, ?, ?)`)
      .run(nonceHash, userId, sessionHash, safeProvider, encryptedCodeVerifier, expiresAt.toISOString());
    backend.db.prepare("DELETE FROM oauth_transactions WHERE datetime(expires_at) < datetime('now', '-1 day')").run();
    backend.db.exec("COMMIT");
  } catch (error) {
    backend.db.exec("ROLLBACK");
    throw error;
...
export async function consumeOAuthTransaction({ nonceHash, userId, sessionHash, provider }) {
  const backend = await getBackend();
  const safeProvider = String(provider).slice(0, 40);
  if (backend.type === "postgres") {
    const rows = await backend.sql`
      UPDATE oauth_transactions SET used_at = NOW()
      WHERE nonce_hash = ${nonceHash} AND user_id = ${userId} AND session_hash = ${sessionHash}
        AND provider = ${safeProvider} AND used_at IS NULL AND expires_at > NOW()
      RETURNING encrypted_code_verifier
    `;
    return rows[0]?.encrypted_code_verifier || null;
  }
  backend.db.exec("BEGIN IMMEDIATE");
  try {
    const row = backend.db.prepare(`SELECT encrypted_code_verifier FROM oauth_transactions
      WHERE nonce_hash = ? AND user_id = ? AND session_hash = ? AND provider = ?
        AND used_at IS NULL AND datetime(expires_at) > CURRENT_TIMESTAMP`)
      .get(nonceHash, userId, sessionHash, safeProvider);
    if (!row) {
      backend.db.exec("COMMIT");
      return null;
    }
    const changed = backend.db.prepare("UPDATE oauth_transactions SET used_at = CURRENT_TIMESTAMP WHERE nonce_hash = ? AND used_at IS NULL")
      .run(nonceHash).changes;
    backend.db.exec("COMMIT");
    return changed ? row.encrypted_code_verifier : null;
  } catch (error) {
    backend.db.exec("ROLLBACK");
    throw error;
  }
...
export async function getUserMfa(userId) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    const rows = await backend.sql`SELECT encrypted_secret, pending_encrypted_secret, pending_expires_at, enabled_at
      FROM user_mfa WHERE user_id = ${userId}`;
    return rows[0] || null;
  }
  return backend.db.prepare(`SELECT encrypted_secret, pending_encrypted_secret, pending_expires_at, enabled_at
    FROM user_mfa WHERE user_id = ?`).get(userId) || null;
}

export async function savePendingUserMfa({ userId, encryptedSecret, expiresAt }) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    await backend.sql`INSERT INTO user_mfa (user_id, pending_encrypted_secret, pending_expires_at)
      VALUES (${userId}, ${encryptedSecret}, ${expiresAt.toISOString()})
      ON CONFLICT (user_id) DO UPDATE SET pending_encrypted_secret = EXCLUDED.pending_encrypted_secret,
        pending_expires_at = EXCLUDED.pending_expires_at, updated_at = NOW()`;
    return;
  }
  backend.db.prepare(`INSERT INTO user_mfa (user_id, pending_encrypted_secret, pending_expires_at)
    VALUES (?, ?, ?) ON CONFLICT(user_id) DO UPDATE SET pending_encrypted_secret = excluded.pending_encrypted_secret,
      pending_expires_at = excluded.pending_expires_at, updated_at = CURRENT_TIMESTAMP`)
    .run(userId, encryptedSecret, expiresAt.toISOString());
}

export async function enableUserMfa({ userId, recoveryCodeHashes }) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    const result = await backend.sql.transaction((tx) => [
      tx`UPDATE user_mfa SET encrypted_secret = pending_encrypted_secret, enabled_at = NOW(),
        pending_encrypted_secret = NULL, pending_expires_at = NULL, updated_at = NOW()
        WHERE user_id = ${userId} AND pending_encrypted_secret IS NOT NULL AND pending_expires_at > NOW()
        RETURNING user_id`,
      tx`DELETE FROM mfa_recovery_codes WHERE user_id = ${userId}`,
      ...recoveryCodeHashes.map((codeHash) => tx`INSERT INTO mfa_recovery_codes (code_hash, user_id)
        SELECT ${codeHash}, ${userId} WHERE EXISTS (SELECT 1 FROM user_mfa WHERE user_id = ${userId} AND enabled_at IS NOT NULL)`),
    ]);
    return Boolean(result[0]?.length);
  }
  backend.db.exec("BEGIN IMMEDIATE");
  try {
    const changed = backend.db.prepare(`UPDATE user_mfa SET encrypted_secret = pending_encrypted_secret,
      enabled_at = CURRENT_TIMESTAMP, pending_encrypted_secret = NULL, pending_expires_at = NULL,
      updated_at = CURRENT_TIMESTAMP WHERE user_id = ? AND pending_encrypted_secret IS NOT NULL
      AND datetime(pending_expires_at) > CURRENT_TIMESTAMP`).run(userId).changes;
    if (!changed) { backend.db.exec("ROLLBACK"); return false; }
    backend.db.prepare("DELETE FROM mfa_recovery_codes WHERE user_id = ?").run(userId);
    const insert = backend.db.prepare("INSERT INTO mfa_recovery_codes (code_hash, user_id) VALUES (?, ?)");
    for (const codeHash of recoveryCodeHashes) insert.run(codeHash, userId);
    backend.db.exec("COMMIT");
    return true;
  } catch (error) {
    backend.db.exec("ROLLBACK");
...
export async function disableUserMfa({ userId, currentSessionHash }) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    await backend.sql.transaction((tx) => [
      tx`DELETE FROM mfa_recovery_codes WHERE user_id = ${userId}`,
      tx`DELETE FROM mfa_login_challenges WHERE user_id = ${userId}`,
      tx`DELETE FROM user_mfa WHERE user_id = ${userId}`,
      tx`UPDATE auth_sessions SET revoked_at = NOW() WHERE user_id = ${userId}
        AND session_hash <> ${currentSessionHash} AND revoked_at IS NULL`,
    ]);
    return;
  }
  backend.db.exec("BEGIN IMMEDIATE");
  try {
    backend.db.prepare("DELETE FROM mfa_recovery_codes WHERE user_id = ?").run(userId);
    backend.db.prepare("DELETE FROM mfa_login_challenges WHERE user_id = ?").run(userId);
    backend.db.prepare("DELETE FROM user_mfa WHERE user_id = ?").run(userId);
    backend.db.prepare("UPDATE auth_sessions SET revoked_at = CURRENT_TIMESTAMP WHERE user_id = ? AND session_hash <> ? AND revoked_at IS NULL")
...
export async function consumeMfaRecoveryCode({ userId, codeHash }) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    const rows = await backend.sql`UPDATE mfa_recovery_codes SET used_at = NOW()
      WHERE user_id = ${userId} AND code_hash = ${codeHash} AND used_at IS NULL RETURNING code_hash`;
    return rows.length > 0;
  }
  return backend.db.prepare(`UPDATE mfa_recovery_codes SET used_at = CURRENT_TIMESTAMP
    WHERE user_id = ? AND code_hash = ? AND used_at IS NULL`).run(userId, codeHash).changes > 0;
}

export async function createMfaLoginChallenge({ challengeHash, userId, expiresAt }) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    await backend.sql.transaction((tx) => [
      tx`UPDATE mfa_login_challenges SET used_at = NOW() WHERE user_id = ${userId} AND used_at IS NULL`,
      tx`INSERT INTO mfa_login_challenges (challenge_hash, user_id, expires_at)
        VALUES (${challengeHash}, ${userId}, ${expiresAt.toISOString()})`,
    ]);
    return;
  }
  backend.db.exec("BEGIN IMMEDIATE");
  try {
    backend.db.prepare("UPDATE mfa_login_challenges SET used_at = CURRENT_TIMESTAMP WHERE user_id = ? AND used_at IS NULL").run(userId);
    backend.db.prepare("INSERT INTO mfa_login_challenges (challenge_hash, user_id, expires_at) VALUES (?, ?, ?)")
      .run(challengeHash, userId, expiresAt.toISOString());
    backend.db.exec("COMMIT");
  } catch (error) {
    backend.db.exec("ROLLBACK");
    throw error;
...
export async function findActiveMfaLoginChallenge(challengeHash) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    const rows = await backend.sql`SELECT user_id, attempt_count FROM mfa_login_challenges
      WHERE challenge_hash = ${challengeHash} AND used_at IS NULL AND expires_at > NOW() AND attempt_count < 5`;
    return rows[0] ? { userId: Number(rows[0].user_id), attemptCount: Number(rows[0].attempt_count) } : null;
  }
  const row = backend.db.prepare(`SELECT user_id, attempt_count FROM mfa_login_challenges
    WHERE challenge_hash = ? AND used_at IS NULL AND datetime(expires_at) > CURRENT_TIMESTAMP AND attempt_count < 5`)
    .get(challengeHash);
  return row ? { userId: Number(row.user_id), attemptCount: Number(row.attempt_count) } : null;
}

export async function failMfaLoginChallenge(challengeHash) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    await backend.sql`UPDATE mfa_login_challenges SET attempt_count = attempt_count + 1,
      used_at = CASE WHEN attempt_count + 1 >= 5 THEN NOW() ELSE used_at END
      WHERE challenge_hash = ${challengeHash} AND used_at IS NULL`;
    return;
  }
  backend.db.prepare(`UPDATE mfa_login_challenges SET attempt_count = attempt_count + 1,
    used_at = CASE WHEN attempt_count + 1 >= 5 THEN CURRENT_TIMESTAMP ELSE used_at END
    WHERE challenge_hash = ? AND used_at IS NULL`).run(challengeHash);
}

export async function consumeMfaLoginChallenge(challengeHash) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    const rows = await backend.sql`UPDATE mfa_login_challenges SET used_at = NOW()
      WHERE challenge_hash = ${challengeHash} AND used_at IS NULL AND expires_at > NOW() AND attempt_count < 5
      RETURNING user_id`;
    return rows[0] ? Number(rows[0].user_id) : null;
  }
  const row = backend.db.prepare(`SELECT user_id FROM mfa_login_challenges WHERE challenge_hash = ?
    AND used_at IS NULL AND datetime(expires_at) > CURRENT_TIMESTAMP AND attempt_count < 5`).get(challengeHash);
  if (!row) return null;
  const changed = backend.db.prepare("UPDATE mfa_login_challenges SET used_at = CURRENT_TIMESTAMP WHERE challenge_hash = ? AND used_at IS NULL")
    .run(challengeHash).changes;
  return changed ? Number(row.user_id) : null;
}

function serializeAuditJson(value, maxBytes = 4_000) {
...
export async function appendAuditEvent({
  ...
  const backend = await getBackend();
  const minimized = serializeAuditJson(metadata);
  const before = previousState === null ? null : serializeAuditJson(previousState);
  const after = newState === null ? null : serializeAuditJson(newState);
  const safeAction = String(action).slice(0, 80);
  const safeOrigin = String(origin || "application").slice(0, 120);
  const safeSubjectType = subjectType ? String(subjectType).slice(0, 60) : null;
  const safeSubjectId = subjectId === null || subjectId === undefined ? null : String(subjectId).slice(0, 120);
  if (backend.type === "postgres") {
    try {
      await backend.sql`
        INSERT INTO audit_events (
          user_id, actor_user_id, organization_id, action, origin, event_version,
          subject_type, subject_id, previous_state, new_state, metadata
        ) VALUES (
          ${userId}, ${actorUserId}, ${organizationId}, ${safeAction}, ${safeOrigin}, ${eventVersion},
          ${safeSubjectType}, ${safeSubjectId}, ${before}::jsonb, ${after}::jsonb, ${minimized}::jsonb
        )
      `;
    } catch (error) {
      // Mantém o deploy funcional enquanto a migration versionada ainda não foi
      // aplicada. Depois da migration, todos os campos enriquecidos são gravados.
      if (error?.code !== "42703") throw error;
      await backend.sql`
        INSERT INTO audit_events (user_id, action, metadata)
        VALUES (${userId}, ${safeAction}, ${minimized}::jsonb)
      `;
    }
    return;
  }
  ...
  `).run(
    userId, actorUserId, organizationId, safeAction, safeOrigin, eventVersion,
    safeSubjectType, safeSubjectId, before, after, minimized,
  );
}
...
function parseAuditJson(value) {
  ...
  try { return JSON.parse(value); } catch { return { unreadable: true }; }
}
...
function serializeAuditEvent(row) {
  return {
    ...
    actor: row.actor_user_id ? {
      ...
      name: row.actor_name || "Conta removida",
      email: row.actor_email || "",
    } : null,
    targetUserId: row.user_id ? String(row.user_id) : null,
    organization: row.organization_id ? {
      id: String(row.organization_id),
      name: row.organization_name || "Empresa removida",
    } : null,
    previousState: parseAuditJson(row.previous_state),
    newState: parseAuditJson(row.new_state),
    metadata: parseAuditJson(row.metadata) || {},
    createdAt: row.created_at,
  };
}

// Consulta global deliberadamente separada das consultas por empresa. Somente a
// API administrativa raiz deve chamar esta função; o cursor evita carregar toda
// a trilha na memória e mantém o custo previsível no Neon.
export async function listAuditEventsForRoot({ cursor = null, limit = 50 } = {}) {
  const backend = await getBackend();
  const safeLimit = Math.min(Math.max(Number(limit) || 50, 1), 100);
  const parsedCursor = cursor && /^\d+$/.test(String(cursor)) ? Number(cursor) : null;
  let rows;
  if (backend.type === "postgres") {
    rows = await backend.sql`
      SELECT ae.*, actor.name AS actor_name, actor.email AS actor_email,
        organization.name AS organization_name
      FROM audit_events ae
      LEFT JOIN users actor ON actor.id = ae.actor_user_id
      LEFT JOIN organizations organization ON organization.id = ae.organization_id
      WHERE (${parsedCursor}::bigint IS NULL OR ae.id < ${parsedCursor})
      ORDER BY ae.id DESC
      LIMIT ${safeLimit + 1}
    `;
  } else {
    rows = backend.db.prepare(`
      SELECT ae.*, actor.name AS actor_name, actor.email AS actor_email,
        organization.name AS organization_name
      FROM audit_events ae
      LEFT JOIN users actor ON actor.id = ae.actor_user_id
      LEFT JOIN organizations organization ON organization.id = ae.organization_id
      WHERE (? IS NULL OR ae.id < ?)
      ORDER BY ae.id DESC
      LIMIT ?
    `).all(parsedCursor, parsedCursor, safeLimit + 1);
  }
  const hasMore = rows.length > safeLimit;
  const items = rows.slice(0, safeLimit).map(serializeAuditEvent);
  return {
    items,
...
function serializeBillingProfile(row, fallbackType = "person") {
  return {
    ...
    setupPaidAt: row?.setup_paid_at || null,
  };
}
...
export async function getBillingProviderState(userId) {
  ...
  const row = backend.type === "postgres"
    ? (await backend.sql`SELECT payment_provider, provider_customer_id, provider_subscription_id, provider_price_id, subscription_status, subscription_current_period_end, setup_paid_at FROM billing_profiles WHERE user_id = ${userId}`)[0]
    : backend.db.prepare("SELECT payment_provider, provider_customer_id, provider_subscription_id, provider_price_id, subscription_status, subscription_current_period_end, setup_paid_at FROM billing_profiles WHERE user_id = ?").get(userId);
  return row ? {
    paymentProvider: row.payment_provider || "",
    ...
    setupPaidAt: row.setup_paid_at || null,
  } : { paymentProvider: "", customerId: "", subscriptionId: "", priceId: "", status: "not_subscriber", currentPeriodEnd: null, setupPaidAt: null };
}
...
export async function getWorkspace(userId, organizationId = null) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    const rows = organizationId == null
      ? await backend.sql`
          SELECT user_id, organization_id, payload, revision, archived_revision,
                 clients_relational_at, tasks_relational_at, deliveries_relational_at,
                 finance_relational_at, updated_at
          FROM workspaces WHERE user_id = ${userId} AND organization_id IS NULL
        `
      : await backend.sql`
          SELECT user_id, organization_id, payload, revision, archived_revision,
                 clients_relational_at, tasks_relational_at, deliveries_relational_at,
                 finance_relational_at, updated_at
          FROM workspaces WHERE user_id = ${userId} AND organization_id = ${organizationId}
        `;
    return hydrateWorkspaceFinance(backend, await hydrateWorkspaceEntities(backend, serializeWorkspace(rows[0])));
  }
  return hydrateWorkspaceFinance(backend, await hydrateWorkspaceEntities(backend, serializeWorkspace(
    organizationId == null
      ? backend.db.prepare("SELECT user_id, organization_id, payload, revision, archived_revision, clients_relational_at, tasks_relational_at, deliveries_relational_at, finance_relational_at, updated_at FROM workspaces WHERE user_id = ? AND organization_id IS NULL").get(userId)
      : backend.db.prepare("SELECT user_id, organization_id, payload, revision, archived_revision, clients_relational_at, tasks_relational_at, deliveries_relational_at, finance_relational_at, updated_at FROM workspaces WHERE user_id = ? AND organization_id = ?").get(userId, organizationId),
  )));
}

export async function listCustomers(ownerUserId, organizationId = null) {
  return listRelationalCustomers(await getBackend(), ownerUserId, organizationId);
}

export async function listOperationalTasks(ownerUserId, organizationId = null) {
  return listRelationalTasks(await getBackend(), ownerUserId, organizationId);
}

export async function listOperationalDeliveries(ownerUserId, organizationId = null) {
  return listRelationalDeliveries(await getBackend(), ownerUserId, organizationId);
}

export async function listFinancialCommitments(ownerUserId, organizationId = null) {
  return listRelationalFinancialCommitments(await getBackend(), ownerUserId, organizationId);
}

export async function listFinancialLedgerEntries(ownerUserId, organizationId = null) {
  return listRelationalFinancialEntries(await getBackend(), ownerUserId, organizationId);
}

async function assertOrganizationOwnerScope(backend, userId, organizationId) {
  if (organizationId == null) return;
  const exists = backend.type === "postgres"
    ? (await backend.sql`SELECT 1 FROM organizations WHERE id = ${organizationId} AND owner_user_id = ${userId} LIMIT 1`).length > 0
    : Boolean(backend.db.prepare("SELECT 1 FROM organizations WHERE id = ? AND owner_user_id = ? LIMIT 1").get(organizationId, userId));
  if (!exists) throw Object.assign(new Error("ORGANIZATION_SCOPE_MISMATCH"), { code: "ORGANIZATION_SCOPE_MISMATCH" });
}

export async function saveWorkspace({ userId, organizationId = null, payload, markSaved = false }) {
  const backend = await getBackend();
  await assertOrganizationOwnerScope(backend, userId, organizationId);
  const serialized = JSON.stringify(payload);

  if (backend.type === "postgres") {
    // Só cria uma nova revisão quando o conteúdo realmente mudou.
    const rows = await backend.sql`
      INSERT INTO workspaces (user_id, organization_id, payload, revision, archived_revision)
      VALUES (${userId}, ${organizationId}, ${serialized}::jsonb, 1, ${markSaved ? 1 : 0})
      ON CONFLICT (user_id) DO UPDATE SET
        ...
        END
      WHERE workspaces.organization_id IS NOT DISTINCT FROM EXCLUDED.organization_id
      RETURNING user_id, organization_id, payload, revision, archived_revision, updated_at
    `;
    if (!rows.length) throw Object.assign(new Error("WORKSPACE_SCOPE_MISMATCH"), { code: "WORKSPACE_SCOPE_MISMATCH" });
    await syncWorkspaceEntities(backend, { ownerUserId: userId, organizationId, payload });
    await syncWorkspaceFinance(backend, { ownerUserId: userId, organizationId, payload });
    return getWorkspace(userId, organizationId);
  }

  const current = await getWorkspace(userId, organizationId);
  const changed = !current || JSON.stringify(current.payload) !== serialized;
  if (!current) {
    backend.db
      .prepare("INSERT INTO workspaces (user_id, organization_id, payload, revision, archived_revision) VALUES (?, ?, ?, 1, ?)")
      .run(userId, organizationId, serialized, markSaved ? 1 : 0);
  } else if (changed) {
    const nextRevision = current.revision + 1;
    backend.db
      .prepare("UPDATE workspaces SET payload = ?, revision = ?, archived_revision = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ? AND organization_id IS ?")
      .run(serialized, nextRevision, markSaved ? nextRevision : current.archived_revision, userId, organizationId);
  } else if (markSaved) {
    backend.db
      .prepare("UPDATE workspaces SET archived_revision = revision WHERE user_id = ? AND organization_id IS ?")
...
export async function archiveWorkspace({ userId, organizationId = null, title }) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    // Cada conta possui um único rascunho contínuo; sair atualiza o mesmo registro.
    const candidates = organizationId == null
      ? await backend.sql`
          UPDATE workspaces SET archived_revision = revision
          WHERE user_id = ${userId} AND organization_id IS NULL AND revision > archived_revision
          RETURNING payload
        `
      : await backend.sql`
          UPDATE workspaces SET archived_revision = revision
          WHERE user_id = ${userId} AND organization_id = ${organizationId} AND revision > archived_revision
          RETURNING payload
        `;
    if (!candidates.length) return null;
    const updated = organizationId == null
      ? await backend.sql`
          UPDATE histories SET title = ${title}, payload = ${JSON.stringify(candidates[0].payload)}::jsonb, created_at = NOW()
          WHERE id = (SELECT id FROM histories WHERE user_id = ${userId} AND organization_id IS NULL AND calculation_type = 'rascunho-automatico' ORDER BY histories.id DESC LIMIT 1)
          RETURNING public_id AS id, title, calculation_type, payload, created_at
        `
      : await backend.sql`
          UPDATE histories SET title = ${title}, payload = ${JSON.stringify(candidates[0].payload)}::jsonb, created_at = NOW()
          WHERE id = (SELECT id FROM histories WHERE user_id = ${userId} AND organization_id = ${organizationId} AND calculation_type = 'rascunho-automatico' ORDER BY histories.id DESC LIMIT 1)
          RETURNING public_id AS id, title, calculation_type, payload, created_at
        `;
    if (updated.length) return updated[0];
    const inserted = await backend.sql`
      INSERT INTO histories (public_id, user_id, organization_id, title, calculation_type, payload)
      VALUES (${randomUUID()}, ${userId}, ${organizationId}, ${title}, 'rascunho-automatico', ${JSON.stringify(candidates[0].payload)}::jsonb)
      RETURNING public_id AS id, title, calculation_type, payload, created_at
    `;
    return inserted[0];
  }
  ...
  try {
    ...
    backend.db
      .prepare("UPDATE workspaces SET archived_revision = revision WHERE user_id = ? AND organization_id IS ?")
      .run(userId, organizationId);
    const existing = backend.db.prepare("SELECT public_id FROM histories WHERE user_id = ? AND organization_id IS ? AND calculation_type = 'rascunho-automatico' ORDER BY histories.id DESC LIMIT 1").get(userId, organizationId);
    const publicId = existing?.public_id || randomUUID();
    if (existing) {
      backend.db.prepare("UPDATE histories SET title = ?, payload = ?, created_at = CURRENT_TIMESTAMP WHERE public_id = ? AND user_id = ? AND organization_id IS ?")
        .run(title, JSON.stringify(current.payload), publicId, userId, organizationId);
    } else {
      backend.db.prepare("INSERT INTO histories (public_id, user_id, organization_id, title, calculation_type, payload) VALUES (?, ?, ?, ?, 'rascunho-automatico', ?)")
        .run(publicId, userId, organizationId, title, JSON.stringify(current.payload));
    }
    backend.db.exec("COMMIT");
    return findHistoryById(publicId, userId, organizationId);
  } catch (error) {
    backend.db.exec("ROLLBACK");
    throw error;
  }
...
function historyPageSize(value) {
  ...
  if (!Number.isFinite(parsed) || parsed < 1) return DEFAULT_HISTORY_PAGE_SIZE;
  return Math.min(parsed, MAX_HISTORY_PAGE_SIZE);
}

function encodeHistoryCursor(id) {
...
function decodeHistoryCursor(value) {
  ...
  try {
    ...
    if (!/^[1-9]\d{0,18}$/.test(decoded)) return null;
    return decoded;
  } catch {
    return null;
  }
...
export async function listHistories(userId, calculationType, { limit, cursor, organizationId = null } = {}) {
  const backend = await getBackend();
  const pageSize = historyPageSize(limit);
  const fetchSize = pageSize + 1;
  const cursorId = decodeHistoryCursor(cursor);
  if (cursor && !cursorId) return { rows: [], nextCursor: null, invalidCursor: true };
  let rows;
  if (backend.type === "postgres") {
    if (calculationType && cursorId) {
      rows = await backend.sql`SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ${userId} AND organization_id IS NOT DISTINCT FROM ${organizationId} AND calculation_type = ${calculationType} AND id < ${cursorId} ORDER BY histories.id DESC LIMIT ${fetchSize}`;
    } else if (calculationType) {
      rows = await backend.sql`SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ${userId} AND organization_id IS NOT DISTINCT FROM ${organizationId} AND calculation_type = ${calculationType} ORDER BY histories.id DESC LIMIT ${fetchSize}`;
    } else if (cursorId) {
      rows = await backend.sql`SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ${userId} AND organization_id IS NOT DISTINCT FROM ${organizationId} AND id < ${cursorId} ORDER BY histories.id DESC LIMIT ${fetchSize}`;
    } else {
      rows = await backend.sql`SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ${userId} AND organization_id IS NOT DISTINCT FROM ${organizationId} ORDER BY histories.id DESC LIMIT ${fetchSize}`;
    }
  } else {
    let statement;
    if (calculationType && cursorId) {
      statement = backend.db.prepare("SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ? AND organization_id IS ? AND calculation_type = ? AND id < ? ORDER BY histories.id DESC LIMIT ?");
      rows = statement.all(userId, organizationId, calculationType, cursorId, fetchSize);
    } else if (calculationType) {
      statement = backend.db.prepare("SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ? AND organization_id IS ? AND calculation_type = ? ORDER BY histories.id DESC LIMIT ?");
      rows = statement.all(userId, organizationId, calculationType, fetchSize);
    } else if (cursorId) {
      statement = backend.db.prepare("SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ? AND organization_id IS ? AND id < ? ORDER BY histories.id DESC LIMIT ?");
      rows = statement.all(userId, organizationId, cursorId, fetchSize);
    } else {
      statement = backend.db.prepare("SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ? AND organization_id IS ? ORDER BY histories.id DESC LIMIT ?");
      rows = statement.all(userId, organizationId, fetchSize);
    }
  }
  const hasMore = rows.length > pageSize;
  const pageRows = rows.slice(0, pageSize);
  const nextCursor = hasMore ? encodeHistoryCursor(pageRows.at(-1).cursor_id) : null;
  return {
    rows: pageRows.map(({ cursor_id: _cursorId, ...row }) => row),
    nextCursor,
    invalidCursor: false,
  };
}
...
export async function saveHistory({ id = null, userId, organizationId = null, title, calculationType, payload }) {
  const backend = await getBackend();
  await assertOrganizationOwnerScope(backend, userId, organizationId);
  if (backend.type === "postgres") {
    if (id) {
      ...
      const updated = await backend.sql`
        ...
        WHERE public_id = ${id} AND user_id = ${userId} AND organization_id IS NOT DISTINCT FROM ${organizationId} AND calculation_type <> 'rascunho-automatico'
        RETURNING public_id AS id, title, calculation_type, payload, created_at
      `;
    ...
    try {
      inserted = await backend.sql`
        INSERT INTO histories (public_id, user_id, organization_id, title, calculation_type, payload)
        SELECT ${randomUUID()}, ${userId}, ${organizationId}, ${title}, ${calculationType}, ${JSON.stringify(payload)}::jsonb
        WHERE (
          SELECT COUNT(*) FROM histories
          WHERE user_id = ${userId} AND organization_id IS NOT DISTINCT FROM ${organizationId} AND calculation_type <> 'rascunho-automatico'
  ...
  try {
    if (id) {
      const updated = backend.db
        .prepare("UPDATE histories SET title = ?, calculation_type = ?, payload = ?, created_at = CURRENT_TIMESTAMP WHERE public_id = ? AND user_id = ? AND organization_id IS ? AND calculation_type <> 'rascunho-automatico'")
        .run(title, calculationType, JSON.stringify(payload), id, userId, organizationId);
      if (updated.changes) {
        const item = await findHistoryById(id, userId, organizationId);
        backend.db.exec("COMMIT");
        return { item, created: false };
      }
    }
    const count = Number(backend.db
      .prepare("SELECT COUNT(*) AS count FROM histories WHERE user_id = ? AND organization_id IS ? AND calculation_type <> 'rascunho-automatico'")
      .get(userId, organizationId)?.count || 0);
    if (count >= MAX_DOCUMENTS_PER_USER) throw documentLimitError();
    const publicId = randomUUID();
    backend.db
      .prepare("INSERT INTO histories (public_id, user_id, organization_id, title, calculation_type, payload) VALUES (?, ?, ?, ?, ?, ?)")
      .run(publicId, userId, organizationId, title, calculationType, JSON.stringify(payload));
    const item = await findHistoryById(publicId, userId, organizationId);
    backend.db.exec("COMMIT");
...
export async function findHistoryById(id, userId, organizationId = null) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    const rows = await backend.sql`
      SELECT public_id AS id, title, calculation_type, payload, created_at
      FROM histories WHERE public_id = ${id} AND user_id = ${userId} AND organization_id IS NOT DISTINCT FROM ${organizationId}
    `;
    return rows[0] || null;
  }
  return backend.db
    .prepare("SELECT public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE public_id = ? AND user_id = ? AND organization_id IS ?")
    .get(id, userId, organizationId) || null;
}

export async function deleteHistory(id, userId, organizationId = null) {
  const backend = await getBackend();
  if (backend.type === "postgres") {
    const rows = await backend.sql`
      DELETE FROM histories WHERE public_id = ${id} AND user_id = ${userId} AND organization_id IS NOT DISTINCT FROM ${organizationId} RETURNING public_id
    `;
    return rows.length > 0;
  }
  return backend.db.prepare("DELETE FROM histories WHERE public_id = ? AND user_id = ? AND organization_id IS ?").run(id, userId, organizationId).changes > 0;
}
