...
export async function buildAccountBackup(userId) {
  ...
  const archive = zipSync({
    "backup-candtech.json": strToU8(json),
    "LEIA-ME.txt": strToU8("EXPORTAÇÃO DE DADOS CANDTECH\nInclui perfil, workspace, documentos de histórico e estoque em JSON.\nNão inclui arquivos anexados, todas as tabelas da plataforma, senhas ou tokens de acesso.\nNão é um snapshot transacional nem um backup completo do Neon/Blob. Não existe importação automática deste ZIP.\nMantenha o arquivo em dispositivo protegido. O ZIP não é cifrado.\n"),
  }, { level: 6 });
