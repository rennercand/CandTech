...
export async function getWorkspace(userId, organizationId = null) {
  ...
  if (backend.type === "postgres") {
    const rows = organizationId == null
      ? await backend.sql`
          SELECT user_id, organization_id, payload, revision, archived_revision, updated_at
  ...
  return serializeWorkspace(
    organizationId == null
      ? backend.db.prepare("SELECT user_id, organization_id, payload, revision, archived_revision, updated_at FROM workspaces WHERE user_id = ? AND organization_id IS NULL").get(userId)
      : backend.db.prepare("SELECT user_id, organization_id, payload, revision, archived_revision, updated_at FROM workspaces WHERE user_id = ? AND organization_id = ?").get(userId, organizationId),
  );
}
