...
export async function createDriveState(userId, redirectUri, historyId = null, sessionHash = "") {
  ...
  return new SignJWT({ purpose: "google-drive", redirectUri, historyId, sessionHash })
...
export async function verifyDriveState(state) {
  ...
  return {
    ...
    historyId: Number.isInteger(payload.historyId) && payload.historyId > 0
      ? payload.historyId
      : null,
...
export async function googleAuthorizationUrl({ userId, redirectUri, historyId, sessionHash }) {
  const state = await createDriveState(userId, redirectUri, historyId, sessionHash);
