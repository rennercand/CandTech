...
function OrderForm({ variants, clients, canDiscount, onSave, busy }) {
  const initial=()=>({ type:"sale",reference:"",partner:"",customerId:"",paymentMethod:"pending",dueOn:"",discountAmount:"",lines:[emptyLine()] });
...
export default function InventoryOperations({ initialSection = "overview", onSnapshot, onDeliveriesChange, clients = [], canExport = false, canUseDrive = false, canDiscount = false, driveStatus }) {
  ...
  const [inventory, setInventory] = useState({ products: [], batches: [], orders: [], lots: [], deliveries: [], insights: { items: [], summary: {} } });
  ...
  const sections = [["overview", "Visão geral"], ["entry", "Entrada rápida"], ["products", "Produtos e variações"], ["import", "Importar planilha"], ["orders", "Pedidos"], ["deliveries", "Entregas"], ["lots", "Lotes e validades"], ["history", "Movimentações"], ["guide", "Como operar"]];
