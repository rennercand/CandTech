...
export async function listHistories(userId, calculationType, { limit, cursor } = {}) {
  const backend = await getBackend();
  ...
  if (backend.type === "postgres") {
    if (calculationType && cursorId) {
      rows = await backend.sql`SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ${userId} AND calculation_type = ${calculationType} AND id < ${cursorId} ORDER BY histories.id DESC LIMIT ${fetchSize}`;
    } else if (calculationType) {
      rows = await backend.sql`SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ${userId} AND calculation_type = ${calculationType} ORDER BY histories.id DESC LIMIT ${fetchSize}`;
    } else if (cursorId) {
      rows = await backend.sql`SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ${userId} AND id < ${cursorId} ORDER BY histories.id DESC LIMIT ${fetchSize}`;
    } else {
      rows = await backend.sql`SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ${userId} ORDER BY histories.id DESC LIMIT ${fetchSize}`;
    }
  } else {
    let statement;
    if (calculationType && cursorId) {
      statement = backend.db.prepare("SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ? AND calculation_type = ? AND id < ? ORDER BY histories.id DESC LIMIT ?");
      rows = statement.all(userId, calculationType, cursorId, fetchSize);
    } else if (calculationType) {
      statement = backend.db.prepare("SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ? AND calculation_type = ? ORDER BY histories.id DESC LIMIT ?");
      rows = statement.all(userId, calculationType, fetchSize);
    } else if (cursorId) {
      statement = backend.db.prepare("SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ? AND id < ? ORDER BY histories.id DESC LIMIT ?");
      rows = statement.all(userId, cursorId, fetchSize);
    } else {
      statement = backend.db.prepare("SELECT id AS cursor_id, public_id AS id, title, calculation_type, payload, created_at FROM histories WHERE user_id = ? ORDER BY histories.id DESC LIMIT ?");
      rows = statement.all(userId, fetchSize);
    }
  }
