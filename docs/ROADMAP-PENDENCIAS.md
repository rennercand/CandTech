...
| --- | --- | --- |
| Autenticação por e-mail, recuperação e sessão revogável | Entregue | `app/api/auth/`, `auth_sessions`, testes de autenticação |
...
| Google Drive e e-mails transacionais | Entregue | OAuth `drive.file`, tokens cifrados e Resend |
...
1. Continuidade, isolamento, auditoria, MFA e validação independente.
