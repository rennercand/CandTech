...
import { getStaffAccessByUserId, listStaffAccess, revokeStaffAccess, saveStaffAccessByEmail } from "@/lib/staff-db";
...
export async function PUT(request) {
  ...
  try {
    ...
    const currentStaff = (await listStaffAccess()).find((item) => item.email.toLowerCase() === email) || null;
    ...
    await appendAuditEvent({
      ...
      origin: "api/admin/staff",
      subjectType: "staff_access",
      subjectId: staff.userId,
      previousState: currentStaff && { canMonitor: currentStaff.canMonitor, canSupport: currentStaff.canSupport, canBilling: currentStaff.canBilling },
...
export async function DELETE(request) {
  ...
  try {
    ...
    const currentStaff = await getStaffAccessByUserId(userId);
    ...
    await appendAuditEvent({
      userId, actorUserId: auth.user.id, action: "staff.access_revoked", origin: "api/admin/staff",
      subjectType: "staff_access", subjectId: userId,
      previousState: currentStaff && { canMonitor: currentStaff.canMonitor, canSupport: currentStaff.canSupport, canBilling: currentStaff.canBilling },
