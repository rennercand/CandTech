...
export default function MonitoringPortal({ administratorName, permissions }) {
  ...
  const [view, setView] = useState(permissions.canMonitor ? "events" : permissions.canSupport ? "tickets" : permissions.canBilling ? "payments" : "staff");
  ...
  return <main className="monitor-page">
    <aside className="monitor-sidebar">
      ...
      <nav aria-label="Seções do monitoramento">
        {permissions.canMonitor && <button className={view === "events" ? "active" : ""} onClick={() => setView("events")}><Icon type="pulse"/>Incidentes</button>}
        {permissions.canSupport && <button className={view === "tickets" ? "active" : ""} onClick={() => setView("tickets")}><Icon type="inbox"/>Mensagens</button>}
        {permissions.canBilling && <button className={view === "payments" ? "active" : ""} onClick={() => setView("payments")}><Icon type="payment"/>Pagamentos Pix{data.totals.reviewPayments ? <span className="monitor-count">{data.totals.reviewPayments}</span> : null}</button>}
