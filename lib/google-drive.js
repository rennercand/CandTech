...
export async function createDriveState(userId, redirectUri, historyId = null) {
  ...
  return new SignJWT({ purpose: "google-drive", redirectUri, historyId })
...
export async function googleAuthorizationUrl({ userId, redirectUri, historyId }) {
  const state = await createDriveState(userId, redirectUri, historyId);
