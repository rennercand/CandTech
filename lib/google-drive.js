...
export async function createDriveState(userId, redirectUri, historyId = null, sessionHash = "", returnTo = "") {
  ...
  return new SignJWT({ purpose: "google-drive", redirectUri, historyId, sessionHash, returnTo: safeReturnTo })
...
export async function googleAuthorizationUrl({ userId, redirectUri, historyId, sessionHash, returnTo = "" }) {
  const state = await createDriveState(userId, redirectUri, historyId, sessionHash, returnTo);
