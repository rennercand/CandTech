...
import { createAuthSession, findActiveAuthSession, findUserById, getUserMfa, revokeAuthSession } from "@/lib/db";
...
export async function createToken(user, { mfaVerified = false } = {}) {
  const sessionId = randomUUID();
  const expiresAt = new Date(Date.now() + 8 * 60 * 60 * 1_000);
  await createAuthSession({ sessionHash: hashSessionId(sessionId), userId: user.id, expiresAt, mfaVerified });
  // JWT é assinado, não cifrado. O perfil é lido do banco em getSession;
  // transportar nome/e-mail aqui só duplicaria dados pessoais no cookie.
  return new SignJWT({})
...
export async function getSession(request, { allowUnverified = false, allowInactiveSubscription = false } = {}) {
  ...
  try {
    ...
    if (typeof payload.jti !== "string" || !payload.jti
      ...
      || !Number.isSafeInteger(Number(payload.sub))) return null;
    ...
    const legalAccepted = Boolean(currentUser.legal_accepted_at)
      && currentUser.terms_version === TERMS_VERSION
      && currentUser.privacy_version === PRIVACY_VERSION;
    if (!allowUnverified && !emailVerified) return null;
    const [billing, mfa] = await Promise.all([getBillingAccess(currentUser.id), getUserMfa(currentUser.id)]);
    if (!allowInactiveSubscription && !billing.active) return null;
    return {
      id: Number(currentUser.id), name: currentUser.name, email: currentUser.email,
      accountType: currentUser.account_type || "person",
      emailVerified,
      legalAccepted,
      subscriptionRequired: billing.required,
      subscriptionActive: billing.active,
      subscriptionStatus: billing.status,
      subscriptionOwnerUserId: billing.ownerUserId,
      isBillingOwner: billing.isBillingOwner,
      mfaEnabled: Boolean(mfa?.enabled_at),
      mfaVerified: Boolean(active.mfaVerifiedAt),
      sessionHash,
    };
  } catch {
    return null;
