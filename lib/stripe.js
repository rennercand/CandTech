...
export function getStripe() {
  ...
  // Impede a mistura de dinheiro real com previews e evita publicar uma chave
  // de teste em produção, dois erros de configuração difíceis de notar na UI.
...
export function stripeSetupPriceId() {
  const value = required("STRIPE_SETUP_PRICE_ID");
  if (!/^price_[A-Za-z0-9]+$/.test(value)) throw new Error("STRIPE_SETUP_PRICE_ID inválido");
  return value;
}
...
export function publicAppUrl() {
  ...
  // URLs de retorno controladas pelo servidor impedem que o Checkout seja
  // usado como redirecionador para um domínio fornecido pelo navegador.
